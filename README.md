...
- **NEXT_PUBLIC_TRANSLATION_ORIGIN**: 可选覆盖翻译文件来源，只填 origin（例如本地开发时的 `http://localhost:8081`），不带 `/files/...`。未设置时使用 `https://translation.exmeaning.com`。影响全部翻译文件（启动时加载的分类译文、活动剧情、卡牌剧情 `cardStory/card_<cardId>.json`、区域对话 `areaTalk/group_<floor(JP actionSet id / 100)>.json`、卡池简介与说明 `gachaInfo.json`）；卡牌剧情与区域对话的译文按日文原文匹配，只在服务器为 JP、界面语言为 zh-CN 或 en-US 且开启翻译时请求，文件不存在时页面保持原文；启动集合缓存在 IndexedDB，切换来源后需要清除站点数据才能看到新来源的内容。
